...
This repository provides functionality for generating unique time ordered identifiers. The specification and the reference implementation are released into the public domain. See the [UNLICENSE](UNLICENSE.md) file.
...
The `UidProvider` generates variable size IDs ranging from 16 bytes to 32 bytes. The identifiers are lexicographically sortable by time when encoded in hex or base32hex format.
...
For global scale, long-term, high-assurance document identification it is recommended to generate 26-byte IDs `(48 bits timestamp + 160 bits random)`. It is a balanced choice for **legal/forensic/archival** use cases where the extremely low collision risk over multi-decade retention is paramount.